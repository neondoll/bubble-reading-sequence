<template>
    <v-card>
        <v-data-table :headers="headers" :items="items" :loading="loading">
            <template v-slot:header.building_construction_id="{ header }">
                <span class="font-inter-normal-700" style="color: #1B202B;">{{ header.text }}</span>
            </template>
            <template v-slot:header.is_kap_remont="{ header }">
                <span class="font-inter-normal-700" style="color: #1B202B;">{{ header.text }}</span>
            </template>
            <template v-slot:header.material="{ header }">
                <span class="font-inter-normal-700" style="color: #1B202B;">{{ header.text }}</span>
            </template>
            <template v-slot:header.obosnovanie="{ header }">
                <span class="font-inter-normal-700" style="color: #1B202B;">{{ header.text }}</span>
            </template>
            <template v-slot:header.srok_eks="{ header }">
                <span class="font-inter-normal-700" style="color: #1B202B;">{{ header.text }}</span>
            </template>
            ...
            <template v-slot:item.building_construction_id="{ item }">
                <span class="font-inter-normal-400" style="color: #1B202B;">
                    {{ getBuildingConstructionValue(item) }}
                </span>
            </template>
            ...
            <template v-slot:item.material="{ item }">
                <span class="font-inter-normal-400" style="color: #1B202B;">{{ item.material }}</span>
            </template>
            <template v-slot:item.obosnovanie="{ item }">
                <span class="font-inter-normal-400" style="color: #1B202B;">{{ item.obosnovanie }}</span>
            </template>
            <template v-slot:item.srok_eks="{ item }">
                <span class="font-inter-normal-400" style="color: #1B202B;">{{ item.srok_eks }}</span>
            </template>
            <template v-slot:top>
                <v-toolbar flat>
...
export default {
    ...
    methods: {
        ...
        getBuildingConstructionValue(item) {
            return this.building_constructions.find(construction => construction.id === item.building_construction_id).value
        },
        async getItems() {
            this.loading = true
