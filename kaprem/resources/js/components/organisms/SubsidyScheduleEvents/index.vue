<template>
    <v-card>
        <v-simple-table v-if="!loading">
            <template v-slot:top>
                <v-toolbar flat>
                    <v-toolbar-title>План-график мероприятий</v-toolbar-title>
                                ...
                                <v-card-text style="padding: 20px 24px;">
                                    <v-tabs>
                                        <template v-for="(item, i) in items">
                                            <v-tab>{{ item.type }}. {{ item.title }}</v-tab>
                                            <v-tab-item>
                                                <v-card flat>
                                                    <v-card-text>
                                                        <v-row>
            ...
            <template v-slot:default>
                ...
                <tr>
                    ...
                    <th><b>Общий фактический объем финансирования (тыс.руб.)</b></th>
                    <th><b>В том числе бюджетное финансирование (тыс. руб.)</b></th>
                    <th><b>В том числе внебюджетное финансирование (тыс. руб.)</b></th>
                    <th><b>Дата загрузки</b></th>
                    <th><b>Принято экспертом</b></th>
                </tr>
                </thead>
                <tbody>
                <template v-for="(item, i) in items">
                    <tr>
                        ...
                        <td>{{ item.date_start ? getDate(item.date_start) : '-' }}</td>
                        <td>{{ item.date_end ? getDate(item.date_end) : '-' }}</td>
                        <td>
                            {{
                        ...
                        <td>{{ item.budget_funding === null ? '-' : Number(item.budget_funding).toFixed(3) }}</td>
                        <td>{{ item.offbudget_funding === null ? '-' : Number(item.offbudget_funding).toFixed(3) }}</td>
                        <td>-</td>
                        <td>-</td>
                    </tr>
                    <template v-for="(document, j) in item.documents">
                        <tr>
                            <td :rowspan="item.documents.length + countFiles(item.documents)" v-if="j === 0">
                                Подтверждающие документы по этапу
                            </td>
                            <td colspan="5">{{ i + 1 }}.{{ j + 1 }}. {{ document.title }}</td>
                            <td></td>
                            <td></td>
                        </tr>
                        <template v-for="(file, k) in document.files">
